...
def categorize_dish(dish_name, dish_ingredients):
    ...
    if dish_ingredients.issubset(VEGAN):
        category = "VEGAN"
    elif dish_ingredients.issubset(VEGETARIAN.union(VEGAN)):
        category = "VEGETARIAN"
...
def tag_special_ingredients(dish):
    ...
    detected_special_ingredients = ingredients.intersection(SPECIAL_INGREDIENTS)
    return dish[0], detected_special_ingredients
...
def separate_appetizers(dishes, appetizers):
    ...
    dishes_without_appetizers: list = []
    for dish in dishes_set:
        if dish not in appetizers_set:
            dishes_without_appetizers.append(dish)
    return dishes_without_appetizers
